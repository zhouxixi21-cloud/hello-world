...
# libmp3lame
DIR=$(mktemp -d) && cd ${DIR} && \
              curl -L -s https://downloads.sourceforge.net/project/lame/lame/${LAME_VERSION%.*}/lame-${LAME_VERSION}.tar.gz | tar zxvf - -C . && \
...
DIR=$(mktemp -d) &&  cd ${DIR} && \
              curl -L -s https://downloads.sourceforge.net/faac/faac-${FAAC_VERSION}.tar.gz | tar zxvf - -C . && \
